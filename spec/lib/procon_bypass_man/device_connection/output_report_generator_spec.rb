...
describe ProconBypassMan::DeviceConnection::OutputReportGenerator do
  ...
  describe '#generate_by_step' do
    it do
      expect(instance.generate_by_step(:home_led_on)).to eq([["01", "00", "00" * 8, "3801"].join].pack("H*"))
      expect(instance.generate_by_step(:home_led_on)).to eq([["01", "01", "00" * 8, "3801"].join].pack("H*"))
    end
  end
