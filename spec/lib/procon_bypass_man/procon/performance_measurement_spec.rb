...
describe ProconBypassMan::Procon::PerformanceMeasurement do
  ...
  describe '.summarize' do
    let(:span_class) { Struct.new(:time_taken, :read_error_count, :write_error_count, :succeed, :interval_from_previous_succeed, :write_time, :read_time, :gc_count, :gc_time) }

    subject { described_class.summarize(spans: spans) }

    context '値があるとき' do
      let(:spans) do
        [ span_class.new(1, 1, 2, true, 1, 0.2, 0.2, 2, 2),
          span_class.new(4, 1, 2, true, 2, 0.1, 0.1, 1, 2),
          span_class.new(2, 1, 2, true, 3, 0.1, 0.1 , 1, 2),
          span_class.new(3, 1, 2, false, 3, 0.1, 0.1, 2, 2),
        ]
      end
